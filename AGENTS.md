...
| `.alvar/visuals/` | SVG, HTML, and markdown previews |
...
7. Visualize only when it improves understanding (`learn-visual`). Put mermaid in a fenced `mermaid` block in the map file. Writing anything under `.alvar/` opens it in Obsidian immediately. For extra figures, call `preview_markdown` or `preview_html`.
...
After writing a mermaid DAG, map, session, knowledge note, or figure under `.alvar/`, Obsidian opens that file immediately. Do not wait to be asked. Still use `preview_markdown` / `preview_html` for dedicated figures.
